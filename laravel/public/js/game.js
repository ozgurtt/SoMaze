...
function sendMove(tileID, el){
	...
	$.getJSON( "/api/v1/solver/move/"+GAME_ID+"/"+tileID+"/"+sessionID, function( data ) {
		...
		if (data.accepted == true){
		    ...
		    else if (data.hp > hp && tileData.tiles[data.tileType].effect.hp > 0){giveAlert("success", "You were healed by a " + getTileName(data.tileType) + " tile and gained health! (" + (data.hp - hp) + " hp)",true);}
		    updatePlayer(data);
		}else{
